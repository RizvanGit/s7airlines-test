...
const Ticket: FC<TicketProps> = ({ fromCity, toCity, flyoutDate }) => {
  const [flightTime, setFlightTime] = useState<typeof flightTimeData.morning>(
    flightTimeData.morning
  )
  ...
  return (
    <>
      <div className="flex flex-col flex-1 w-full even:border-t-2 border-dotted">
        <div className="px-4 py-[0.5px] text-sm bg-cyan-600 text-white w-[130px] rounded-tl-xl rounded-br-xl">
          Нeвозвратный
        </div>
        <div className="flex px-4 py-2 flex-col md:flex-row">
          ...
          <div className="flex-1 flex flex-col gap-5 relative">
            <div className="flex">
              <div className="flex-1 flex gap-5 flex-col sm:flex-row justify-center">
                <div className="text-center">
                  <div className="text-2xl font-bold sm:w-[80px]">
                    {flightTime.start}
                  </div>
            ...
            <div className="flex gap-3">
              <Button
                id="morning"
                onClick={onChangeFlightTimeHandler}
                variant="outline"
                className="px-2 py-1 text-sm font-medium border-2 border-gray-200 rounded-xl"
              >
                {flightTimeData.morning.start} -{" "}
                <span className="text-xs">{flightTimeData.morning.end}</span>
              </Button>
              <Button
                id="afternoon"
                onClick={onChangeFlightTimeHandler}
                ...
                className="px-2 py-1 text-sm font-medium border-2 border-gray-200 rounded-xl"
              >
                {flightTimeData.afternoon.start} -{" "}
                <span className="text-xs">{flightTimeData.afternoon.end}</span>
              </Button>
              <Button
                id="evening"
                onClick={onChangeFlightTimeHandler}
                ...
                className="px-2 py-1 text-sm font-medium border-2 border-gray-200 rounded-xl"
              >
                {flightTimeData.evening.start} -{" "}
                <span className="text-xs">{flightTimeData.evening.end}</span>
              </Button>
            </div>
          </div>
        </div>
